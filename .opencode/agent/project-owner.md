...
How to operate:
- "What should we build next?" → read spec §28; the next engine milestone is step 14 (weighted scheduling/bonding); steps 11/12/15/16 need hardware/platform (live NICs, Tauri, WFP, Wordlyte). State scope, dependencies, and what "done" means.
...
- "Track status" → summarize committed milestones (probes `8b88eaf`, failover `1073978`, adaptive duplication `9086e0f`, reorder `bb2d06e`), remaining steps, and risks.
